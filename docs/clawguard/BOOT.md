# ClawGuard — Boot Context

This document defines the core vision, purpose, and mental model of ClawGuard. It should be passed to every agent, contributor, or AI assistant working on this project to ensure a shared understanding.

---

## What is ClawGuard?

ClawGuard is the **enterprise admin console and control plane** for [OpenClaw](https://github.com/openclaw), an open-source AI assistant. It gives organizations centralized governance over every OpenClaw instance running across their employees' machines.
...
ClawGuard solves this by providing a **single admin panel** that connects to every employee's OpenClaw instance and gives the organization:
...
| Actor | Description |
|---|---|
| **Org Admin** | Uses the ClawGuard admin console (web UI) to manage policies, review skills, query audits, and control the kill switch. Has full visibility over all connected OpenClaw instances. |
| **Employee / User** | Runs OpenClaw on their machine for day-to-day work. Their instance connects to the org's ClawGuard control plane and follows the policies set by the admin. They can submit skills for approval and see their own status. |
| **OpenClaw Instance** | The AI assistant running on an employee's machine. It connects to ClawGuard via the `@openclaw/clawguard` plugin, authenticates the user, fetches policies, enforces tool restrictions, uploads audit events, and checks in via heartbeat. |
...
1. Admin sets up ClawGuard (server + admin console) and creates the organization.
2. Each employee installs the ClawGuard plugin in their OpenClaw config and authenticates (via SSO or email/password).
...
|---|---|---|
| `@openclaw/clawguard` | OpenClaw plugin — hooks into the AI assistant lifecycle, enforces policies client-side, uploads audit events, polls heartbeat | Employee's machine |
| `@openclaw/clawguard-server` | Control plane API — manages auth, policies, skill reviews, audit storage, heartbeat, kill switch | Org's server / cloud |
| `@openclaw/clawguard-admin` | Admin web console — UI for managing everything | Org's server / cloud (accessed via browser) |
...
### Organization
The top-level tenant. An org groups users, policies, skills, and audit logs. Everything in ClawGuard is scoped to an org.
...
### Enrollment
How an employee's OpenClaw instance joins the org. Currently via SSO/OIDC login (`/clawguard-login`). The user authenticates, the control plane links them to the org, and the plugin stores a session locally.
...
- **Not a replacement for OpenClaw** — It adds governance on top of OpenClaw. Without OpenClaw, ClawGuard has nothing to govern.
...
1. **Admin has control, user has transparency.** The admin governs; the user can always see what policies apply to them (`/clawguard-status`).
